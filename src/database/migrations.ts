...
export class MigrationManager {
  private migrations: Migration[] = [
    ...
    {
      version: 2,
      name: 'add_transcription_fields',
      up: () => {
        // Add transcription fields to episodes table
        try {
          db.exec(`
            ALTER TABLE episodes 
            ADD COLUMN transcription_status TEXT DEFAULT 'none' 
            CHECK (transcription_status IN ('none', 'queued', 'transcribing', 'completed', 'failed'))
          `);
        } catch (error) {
          // Column might already exist
          console.log('transcription_status column may already exist');
        }

        try {
          db.exec(`ALTER TABLE episodes ADD COLUMN transcription_path TEXT`);
        } catch (error) {
          console.log('transcription_path column may already exist');
        }

        try {
          db.exec(`ALTER TABLE episodes ADD COLUMN transcription_error TEXT`);
        } catch (error) {
          console.log('transcription_error column may already exist');
        }

        // Create index for transcription status
        db.exec(`CREATE INDEX IF NOT EXISTS idx_episodes_transcription_status ON episodes(transcription_status)`);
        
        console.log('✅ Added transcription fields to episodes table');
      },
      down: () => {
        // SQLite doesn't support DROP COLUMN, so we'd need to recreate the table
        console.log('⚠️ Rollback for transcription fields not implemented (SQLite limitation)');
      },
    },
  ];
