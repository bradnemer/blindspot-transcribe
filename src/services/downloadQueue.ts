  ...
  private async startDownload(episode: Episode): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        // Queue file for transcription if download was successful
        if (result.filePath) {
          try {
            console.log(`📝 Queueing transcription for: ${episode.episode_title}`);
            
            // Update episode with transcription queued status
            if (episode.id) {
              dal.episodes.update(episode.id, {
            ...
            await transcriptionService.queueTranscription(result.filePath);
          } catch (transcriptionError) {
            console.error('Failed to queue transcription:', transcriptionError);
            // Don't fail the download if transcription queueing fails
            if (episode.id) {
              dal.episodes.update(episode.id, {
                transcription_status: 'failed',
                transcription_error: `Failed to queue: ${transcriptionError}`
              });
            }
          }
        }
