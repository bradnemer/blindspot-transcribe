...
export class Downloader {
  ...
  private activeDownloads = new Map<number, CancelTokenSource>();
   ...
   */
  public cancelDownload(episodeId: number): boolean {
   ...
   */
  public isDownloading(episodeId: number): boolean {
   ...
   */
  public async testUrl(url: string): Promise<{
    accessible: boolean;
   ...
   */
  private cleanupFailedDownload(episode: Episode): void {
    try {
      this.fileManager.cleanupFailedDownload(episode);
      this.activeDownloads.delete(episode.episode_id);
      this.downloadStartTimes.delete(episode.episode_id);
