...
import { Episode, CSVRow } from '../types';
...
export class CSVParser {
   ...
   */
  public static validateHeaders(headers: string[]): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  private static convertRowToEpisode(row: CSVRow): Episode {
    return {
   ...
   */
  private static checkForDuplicates(episodes: Episode[]): {
   ...
   */
  public static parseText(csvText: string): ParseResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    const episodes: Episode[] = [];
    let totalRows = 0;
