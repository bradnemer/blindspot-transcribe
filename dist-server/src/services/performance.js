...
export class PerformanceMonitor {
    ...
    maxMetricsPerOperation = 100; // Keep last 100 measurements per operation
    static getInstance() {
    ...
    recordMetric(name, startTime, endTime, metadata) {
        const metric = {
            name,
            startTime,
            endTime,
...
export function timedAsync(name) {
    return function (target, propertyKey, descriptor) {
        const originalMethod = descriptor.value;
        const operationName = name || `${target.constructor.name}.${propertyKey}`;
