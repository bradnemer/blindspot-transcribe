        ...
        writer.on('finish', () => {
          try {
            ...
            if (stats.size > 0) {
              ...
              const finalUpdateStmt = db.prepare('UPDATE episodes SET download_status = ?, download_progress = ?, file_path = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
              finalUpdateStmt.run('downloaded', 100, filePath, episode.id);
...
app.post('/api/downloads/sync', (req, res) => {
  try {
    ...
    for (const episode of episodes) {
      ...
      if (fs.existsSync(expectedPath)) {
        ...
        if (stats.size > 0) {
          ...
          const updateStmt = db.prepare('UPDATE episodes SET download_status = ?, download_progress = ?, file_path = ?, retry_count = 0, error_message = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
          updateStmt.run('downloaded', 100, expectedPath, episode.id);
